...
const BorderBox10 = ({ options }: IWidgetProps) => {
  const { width, height, titleWidth, title } = options;
  const filterId = useRef(`border-box-11-filterId-${guid()}`).current;

  const mergedColor = defaultColor;
  const backgroundColor = 'transparent';
  const classNames = 'dv-border-box-11';
  ...
  return (
    <div className={classNames}>
      <svg className='dv-border-svg-container' width={width} height={height}>
        ...
        <text className='dv-border-box-11-title' x={`${width / 2}`} y='32' fill='#fff' fontSize='18' textAnchor='middle' dominantBaseline='middle'>
          {title}
        </text>
