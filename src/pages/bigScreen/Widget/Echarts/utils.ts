...
export function handleEchartsOption(style: any): any {
  return {
    ...
    gauge: {
      series: {
        type: 'gauge',
        clockwise: style?.gaugeClockwise,
        startAngle: style?.gaugeStartAngle,
        endAngle: style?.gaugeEndAngle,
        center: [
          `${style?.gaugeCenterX || 0}%`,
          `${style?.gaugeCenterY || 0}%`
        ],
        title: {
          show: style?.gaugeShowTitle,
        ...
        progress: {
          show: true,
          width: style?.gaugeAxisLineWidth || 10,
          itemStyle: {
            color: style?.themeColor1
          }
        },
        axisLine: {
          lineStyle: {
            width: style?.gaugeAxisLineWidth || 10,
            color: style?.gaugeAxisLineColor || [
              [1, colord(style?.themeColor1).alpha(0.7).toRgbString()]
            ]
          }
        },
        splitLine: {
          show: style?.gaugeAxisTickShow
        },
        axisTick: {
          show: style?.gaugeAxisTickShow,
          lineStyle: {
        ...
        detail: {
          color: style?.styleColor || style?.themeColor1,
          fontStyle: style?.styleFontStyle || 'normal',
          fontWeight: style?.styleFontWeight || 'bold',
          fontFamily: style?.styleFontFamily || 'sans-serif',
          fontSize: style?.styleFontSize || 30,
          lineHeight: style?.styleLineHeight || 30,
          formatter: style?.gaugeDetailFormatter || '{value}',
          padding: style?.styleLetterSpacing || 0,
          valueAnimation: true,
          offsetCenter: [
            `${style?.gaugeDetailOffsetCenterX || 0}%`,
            `${style?.gaugeDetailOffsetCenterY || 40}%`
          ]
        },
        anchor: {
          show: style?.gaugeArchorShow,
          showAbove: true,
          size: 15,
          itemStyle: {
            borderWidth: 5,
